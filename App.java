...
public class App {
    ...
    */
    static int menu() {
        ...
        System.out.println("9 - Mostrar pedidos do cliente");
    ...
    static AVL<Integer, Cliente> lerClientes(String nomeArquivoDados) {
        ...
        try {
    		...
    		quantosClientes = Integer.parseInt(arquivo.nextLine());
    ...
    private static void inserirNaTabelaPedidosDoCliente(Cliente cliente, Pedido pedido) {
        ...
        try {
            ...
            pedidos.pesquisar(pedido);

        } catch (NoSuchElementException e) {

            if (pedidos == null || pedidos.vazia()) pedidos = new Lista<>();
    ...
    public static void pedidosDoCliente() {
        ...
        Cliente c = clientesPorId.pesquisar(documento);
        
        try { 
            
            Lista<Pedido> l = pedidosPorCliente.pesquisar(c);

            StringBuilder sb = new StringBuilder();

            sb.append("Cliente: \n" + c.toString() + "\nPedidos:");
            l.paraCada( p -> sb.append(p.toString()) );

        }
        catch ( NoSuchElementException e ) { System.out.println("Nenhum pedido registrado"); }

    }

    /**
     * TODO: implementar.
     * Percorre a tabela hash pedidosPorCliente e produz um relatório listando todos os
     * clientes que possuem ao menos dois pedidos, exibindo, para cada um: nome, documento,
     * quantidade de pedidos e valor total acumulado dos pedidos. O relatório deve ser
     * apresentado em ordem decrescente de valor total.
     * Dica: a classe TabelaHash oferece o método paraCada(BiConsumer), que permite executar
     * uma ação para cada par chave/valor armazenado, sem que você precise acessar diretamente
     * sua estrutura interna.
     */
    public static void rankingClientes() {

        AVL<Integer, Cliente> aux = new AVL<>();

        for ( int i = 1; i < quantosClientes; i ++ ) {

            int id = 10_000 + i;

            Cliente c = clientesPorId.pesquisar(id);
            Lista<Pedido> l = pedidosPorCliente.pesquisar(c);

            if ( l.tamanho() > 1 ) aux.inserir(id, c);
